...
This addendum modifies and strengthens the architectural requirements of the **Alone the Lambdas — Greenfield Core Language Specification**.
...
If the Alone the Lambdas language contains a List, Nat, Bool, Error, Result, Char, or String, that value must genuinely be represented by the lambda encodings defined by the project.
...
This is implementation scaffolding, not computational machinery available to Alone the Lambdas programs.
