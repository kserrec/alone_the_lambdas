...
## Phase 29 prepublication contract
...
| Temporary native staging is narrow and one-time. | Kyle separately approved only the Phase 29 transfer of two final-but-unpublished macOS archives and one final-but-unpublished Windows archive, their one-entry checksums, and self-contained consumers. Linux remains within one job. After all consumers pass, the three GitHub artifacts may remain only until their exact tested bytes are downloaded and verified locally, then must be deleted through the API; one-day retention is the automatic fallback. |
...
| Language computation and authority do not change. | No core, effect, runtime, reader, macro, expander, or runner source changes. The intended executable-visible change is only `attalambda --version`; package metadata, final archive wording, build manifests, tests, CI staging, and current documentation change around it. Local Racket CS 9.3 verification passed 4,751 assertions across all 32 test files, the unchanged expanded purity proof over 16 `core/` modules, and the complete zero-finding boundary/source inventory. Object-language computation and the sole language-visible `host` boundary remain unchanged. |
