...
## Phase 29 final-but-unpublished evidence
...
| The exact final archives are staged but unpublished. | The [Phase 29 staging record](design/standalone-distribution.md#phase-29-staging-record) records all four names, byte counts, SHA-256 values, inventories, dependencies, signing states, and observed platforms. Their exact 410-byte combined `SHA256SUMS` has SHA-256 `7786bf553caac0087ab22f3636d546a1fe00f89a446611c1516cc58f411f6f7f`; all four entries pass fresh local verification. The Linux archive was deterministically reproduced to the exact CI hash and passed the pinned no-Racket consumer again. |
...
| Publication remains a separate external mutation. | The implementation and transfer approvals do not authorize paid GitHub usage, a Git tag, GitHub Release, signing, release-asset upload, public-download claim, or publication. Those actions remain blocked until the exact tested commit, files, combined checksum manifest, observed platforms, signing state, limitations, and public consequences are presented and explicitly approved. |
...
and Phase 29 prepares final `0.2.0` artifacts for a separate exact publication
decision. Source promotion and temporary staging do not establish a public
download, a
