...
| Requirement | Evidence |
| --- | --- |
| Exact approved legal payloads replaced the transitional marker. | All three Phase 28 builders packaged root [`LICENSE`](../LICENSE), whose SHA-256 was and remains `cfc7749b96f63bd31c3c42b5c471bf756814053e847c10f3eb003417bc523d30`, plus the then-current reviewed 100,029-byte notice payload, whose SHA-256 was `1343f218ba484a79fbef498d4e8fb02e202763a19e46c5e610a8bfe900bcbefd`. They pinned and verified that digest before packaging, recorded both hashes in the manifest, and excluded the historical development-warning input. Consumers independently hashed the extracted bytes and rejected the old marker. Phase 29 changes only the notice's AttaLambda version heading; its exact current bytes are recorded below. |
...
| The approved transfer left nothing downloadable. | Linux remained in one job. Only the two macOS archives and one Windows archive, their one-entry checksums, and self-contained consumer harnesses crossed the separately approved temporary boundary. One-day retention was only a cleanup-failure fallback. Both cleanup jobs passed, and the completed run's artifact API returned `total_count: 0`. At Phase 28 completion, no tag, GitHub Release, signing operation, public download, or publication existed. |
...
## Phase 29 final-release evidence

| Requirement | Evidence |
| --- | --- |
| Final version metadata is exact. | Root [`VERSION`](../VERSION) contains exactly `0.2.0` plus LF and [`info.rkt`](../info.rkt) carries its approved Racket projection `0.2`. Runner source remains unchanged and derives `AttaLambda 0.2.0`; builders derive the four final archive roots and filenames from the same source. |
| The final notice changes no bundled legal term. | Current [`THIRD_PARTY_NOTICES.md.in`](../distribution/THIRD_PARTY_NOTICES.md.in) is exactly 100,024 bytes with SHA-256 `516b3a08454709bf111494c92ed260a5c4afb47c91d06efca924b500c89e17ad`. Its only difference from the exact Phase 28-approved file is the AttaLambda-specific heading version `0.2.0-rc.1` → `0.2.0`; every bundled component inventory entry, notice, and license term is unchanged. The repository license bytes and digest are unchanged. |
| One exact commit passed every required gate. | Source commit `42ff0a7810ebeced445ab23561433a2dc423e433` passed the complete suite, all four native builds, and all four independent no-Racket consumers in [run 33262922610](https://github.com/kserrec/attalambda/actions/runs/33262922610). Local and CI source verification passed 4,751 assertions across 32 files, the unchanged 16-module purity proof, and the complete zero-finding boundary/source inventory. Every consumer passed the printed guide workflow, relocation, and final acceptance. |
| The exact final archives were originally published and verified. | The [Phase 29 staging record](design/standalone-distribution.md#phase-29-staging-record) records all four names, byte counts, SHA-256 values, inventories, dependencies, signing states, and observed platforms. Their exact 410-byte combined `SHA256SUMS` has SHA-256 `7786bf553caac0087ab22f3636d546a1fe00f89a446611c1516cc58f411f6f7f`; all four entries pass fresh local verification. The [Phase 29 publication record](design/standalone-distribution.md#phase-29-publication-record) records the separately approved tag and original public Release. Every original public file was downloaded anonymously and matched staging byte-for-byte; the anonymous Linux archive also passed the full pinned no-Racket consumer again. Phase 30 below supersedes only the current public platform surface. |
| Temporary native staging was narrow and one-time. | Linux remained within one job. Only the two macOS archives and one Windows archive, their one-entry checksums, and self-contained consumers crossed jobs. After all three consumers passed, Codex downloaded and verified the exact tested bytes, deleted artifact IDs `9717803836`, `9717807355`, and `9717796683` through the API, and verified `total_count: 0`. One-day retention was only the automatic fallback; ordinary `always()` cleanup is restored. |
| Signing state is literal. | No identity-backed signing, detached signing, or notarization operation was performed. Windows reported Authenticode `NotSigned`; staged Mach-O inspection found only the arm64 toolchain's automatic ad-hoc linker signature and no code-signature load command on x86-64. No detached cryptographic signature accompanies any archive or `SHA256SUMS`. |
| Publication was separately authorized and verified. | Kyle separately approved the exact unsigned annotated tag `v0.2.0` at source commit `42ff0a7810ebeced445ab23561433a2dc423e433`, the latest non-prerelease public Release titled `AttaLambda 0.2.0`, and manual upload of only the four named archives plus `SHA256SUMS`. The tag, draft, assets, public API and HTML, latest-release resolution, and every anonymous public download were then verified exactly as recorded. GitHub supplied only its automatic source-code links in addition to the five manual assets; no paid feature, purchase, GitHub Actions run, identity-backed or detached signing, or notarization was used. |
| Language computation and authority do not change. | No core, effect, runtime, reader, macro, expander, or runner source changed. The intended executable-visible change is only `attalambda --version`; package metadata, final archive wording, build manifests, tests, CI staging, and current documentation changed around it. Object-language computation and the sole language-visible `host` boundary remain unchanged. |

## Phase 30 supported-release correction evidence

| Requirement | Evidence |
| --- | --- |
| Public support follows the real downloaded-user experience. | A public macOS archive downloaded and launched on a consumer Mac produced Gatekeeper's malware-verification block, offering only trash or dismissal. The Phase 29 macOS consumer proved isolated execution after an Actions artifact transfer, but did not apply browser quarantine metadata or assess Gatekeeper. The executable therefore worked in the tested harness while the actual public first-run path failed. |
| The untested Windows consumer-security path is not presented as support. | The final Windows executable's observed Authenticode status is exactly `NotSigned`. Microsoft's current [SmartScreen developer guidance](https://learn.microsoft.com/en-us/windows/apps/package-and-deploy/smartscreen-reputation) says an unsigned download receives “Windows protected your PC,” requires “Run anyway,” and may be non-bypassable under enterprise policy. Microsoft's [Smart App Control guidance](https://support.microsoft.com/en-us/windows/security/threat-malware-protection/smart-app-control-frequently-asked-questions) says Windows 11 treats an unsigned app as untrusted and blocks it when cloud prediction cannot establish safety. No Windows client-edition downloaded-user test had been performed. |
| The public Release exposes only the demonstrated target. | Release ID `379061612` retains `attalambda-0.2.0-linux-x86_64.tar.gz` (asset ID `535549598`) and the original `SHA256SUMS` (asset ID `535549605`). The two macOS assets (`535549609`, `535549602`) and Windows asset (`535549611`) were deleted. The tag, Release identity, Linux bytes, and GitHub-generated source archives were not replaced. |
| Published integrity history is not silently rewritten. | The exact 410-byte `SHA256SUMS` remains byte-for-byte unchanged and therefore retains the original four archive entries as historical evidence, not current support claims. Verified byte-identical local copies of all three withdrawn assets remain available for forensic recovery, but re-upload requires a new explicit decision. |
| Linux still passes the outsider workflow. | In a fresh Ubuntu 24.04 Docker userspace on x86-64, Kyle downloaded the two public files from their release URLs, verified `attalambda-0.2.0-linux-x86_64.tar.gz: OK`, confirmed that Racket was absent, observed `AttaLambda 0.2.0`, and ran the bundled hello example to obtain `Hello from AttaLambda.`. This proves the documented Linux target, not a broader distribution or architecture floor. |
| The correction changes delivery claims only. | Current user documentation and its 207 focused distribution assertions now name Linux x86-64 as the sole supported binary target. The complete suite passes 4,755 assertions across all 32 files, the unchanged 16-module purity proof, and the zero-finding boundary/source inventory. Historical phase records, internal macOS/Windows portability builders, CI jobs, legal notices, and the original publication manifest remain. No production-language, runner, runtime, effect, reader, macro, or expander file changes. |
...
archives, Phase 28 proves unpublished self-contained `0.2.0-rc.1` candidates,
and Phase 29 originally published the exact final `0.2.0` artifacts under a
separate explicit approval. Phase 30 withdraws the macOS and Windows public
assets after the untested operating-system security paths were surfaced; only
Linux x86-64 remains a supported public binary target. Neither phase
establishes a Linux compatibility floor below the exact Ubuntu 24.04 consumer,
a signed artifact, installer behavior, or a support policy. The language has no
...
| Rational arithmetic is exact, reduced, single-zero, and pure. | [`rat-test.rkt`](../tests/rat-test.rkt) matches Racket's exact rationals across the 16×16 operand matrix with stored-part verification; [`typed-rat-test.rkt`](../tests/typed-rat-test.rkt) covers the strict layer; the expanded purity scan covers all 22 core modules. |
| Division, reciprocal, and powers fail explicitly. | Kind‑3 DivideByZero and kind‑13 NonWholeExponent Results in [`rat-test.rkt`](../tests/rat-test.rkt), including rejected fractional exponents whose powers would have been rational. |
| Whole-number consumers ride Rat. | LEN/TAKE/DROP/MAKE-CHAR/STRING-LENGTH in [`rat-test.rkt`](../tests/rat-test.rkt) and [`errors-test.rkt`](../tests/errors-test.rkt); ports, backlogs, limits, and handles in [`tcp-test.rkt`](../tests/tcp-test.rkt) and [`tcp-host-test.rkt`](../tests/tcp-host-test.rkt) with INVALID-COUNT (kind 14) rejection before any host call. |
