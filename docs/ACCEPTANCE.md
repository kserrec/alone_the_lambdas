...
| The exact final archives are publicly available. | The [Phase 29 staging record](design/standalone-distribution.md#phase-29-staging-record) records all four names, byte counts, SHA-256 values, inventories, dependencies, signing states, and observed platforms. Their exact 410-byte combined `SHA256SUMS` has SHA-256 `7786bf553caac0087ab22f3636d546a1fe00f89a446611c1516cc58f411f6f7f`; all four entries pass fresh local verification. The [Phase 29 publication record](design/standalone-distribution.md#phase-29-publication-record) records the separately approved tag and public Release. Every public file was downloaded anonymously and matched staging byte-for-byte; the anonymous Linux archive also passed the full pinned no-Racket consumer again. |
...
and Phase 29 publishes the exact final `0.2.0` artifacts under a separate
explicit approval. That publication does not establish a compatibility floor
below the exact demonstrated consumers, Windows client-edition support, a
signed artifact, installer behavior, or a support policy. The language has no
